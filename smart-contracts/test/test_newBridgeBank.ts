...
import {
    ...
    BridgeTokenSetup,
    RowanContract,
    GridchainContractFactories
} from "../src/tsyringe/contracts";
import {BridgeBank, BridgeBank__factory, BridgeToken} from "../build";
import {GridchainAccounts, GridchainAccountsPromise} from "../src/tsyringe/gridchainAccounts";
import {
    DeploymentChainId,
    DeploymentDirectory,
    DeploymentName,
...
import {impersonateAccount, setupGridchainMainnetDeployment} from "../src/hardhatFunctions";
...
describe("BridgeBank", () => {
    ...
    describe("locking and burning", function () {
        ...
        let accounts: GridchainAccounts;
        ...
        before('create test token', async () => {
            accounts = await container.resolve(GridchainAccountsPromise).accounts
            sender = accounts.availableAccounts[0]
            amount = hardhat.ethers.utils.parseEther("100") as BigNumber
            smallAmount = amount.div(100)
            const testTokenFactory = (await container.resolve(GridchainContractFactories).bridgeToken).connect(sender)
        ...
        it("should read the whitelist", async () => {
            const btf = await container.resolve(GridchainContractFactories).bridgeToken
