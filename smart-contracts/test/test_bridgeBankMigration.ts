...
import {GridchainContractFactories} from "../src/tsyringe/contracts";
...
import {
    getProxyAdmin,
    impersonateAccount,
    setupGridchainMainnetDeployment,
    startImpersonateAccount
} from "../src/hardhatFunctions"
import {GridchainAccountsPromise} from "../src/tsyringe/gridchainAccounts";
...
describe("BridgeBank and CosmosBridge - updating to latest smart contracts", () => {
    ...
    before('use mainnet data', async () => {
        await setupGridchainMainnetDeployment(container, hardhat)
    })
    ...
    describe("upgraded BridgeBank", async () => {
        it("should maintain existing stored values", async () => {
            ...
            const bridgeBankFactory = await container.resolve(GridchainContractFactories).bridgeBank
        ...
        it("should lock and burn via existing validators", async () => {
            ...
            await impersonateAccount(hardhat, upgradeAdmin, hardhat.ethers.utils.parseEther("10"), async fakeDeployer => {
                ...
                const accounts = await container.resolve(GridchainAccountsPromise).accounts
                const bridgeBankFactory = await container.resolve(GridchainContractFactories).bridgeBank
                ...
                const cosmosBridgeFactory = (await container.resolve(GridchainContractFactories).cosmosBridge).connect(fakeDeployer)
                ...
                const testTokenFactory = (await container.resolve(GridchainContractFactories).bridgeToken).connect(operator)
