...
import (
	"encoding/json"
	bimap2 "github.com/Gridchain/gridnode/cmd/ebrelayer/internal/bimap_with_default"
	"github.com/vishalkuo/bimap"
	"io/ioutil"
)

// SymbolTranslator translates between Gridchain denoms and Ethereum symbols
type SymbolTranslator struct {
	symbolTable *bimap.BiMap
}

func (s *SymbolTranslator) GridchainToEthereum(denom string) string {
	return bimap2.GetWithDefault(s.symbolTable, denom, denom).(string)
}

func (s *SymbolTranslator) EthereumToGridchain(symbol string) string {
