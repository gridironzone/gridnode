...
import (
	"errors"
	"github.com/Gridchain/gridnode/cmd/ebrelayer/internal/symbol_translator"
	...
	"github.com/Gridchain/gridnode/cmd/ebrelayer/types"
	ethbridge "github.com/Gridchain/gridnode/x/ethbridge/types"
)

const (
	nullAddress           = "0x0000000000000000000000000000000000000000"
	defaultGridchainPrefix = "c"
)
...
func EthereumEventToEthBridgeClaim(valAddr sdk.ValAddress, event types.EthereumEvent, symbolTranslator *symbol_translator.SymbolTranslator, sugaredLogger *zap.SugaredLogger) (ethbridge.EthBridgeClaim, error) {
	...
	case ethbridge.ClaimType_CLAIM_TYPE_BURN:
		symbol = symbolTranslator.EthereumToGridchain(symbol)
	}
...
func BurnLockEventToCosmosMsg(claimType types.Event, attributes []abci.EventAttribute, symbolTranslator *symbol_translator.SymbolTranslator, sugaredLogger *zap.SugaredLogger) (types.CosmosMsg, error) {
	...
	for _, attribute := range attributes {
		...
		case types.Symbol.String():
			...
			switch claimType {
			case types.MsgLock:
				symbol = symbolTranslator.GridchainToEthereum(val)
			case types.MsgBurn:
				if !strings.Contains(val, defaultGridchainPrefix) {
					// log.Printf("Can only relay burns of '%v' prefixed coins", defaultGridchainPrefix)
					sugaredLogger.Errorw("only relay burns prefixed coins", "coin symbol", val)
					return types.CosmosMsg{}, errors.New("can only relay burns of '%v' prefixed coins" + defaultGridchainPrefix)
				}
				res := strings.SplitAfter(val, defaultGridchainPrefix)
