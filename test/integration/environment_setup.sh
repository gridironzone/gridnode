...
required="${ROWAN_SOURCE:?"Must be set to a grid address that contains rowan, and the key must be in your keyring"}"
...
cp $BASEDIR/smart-contracts/build/contracts/GridchainTestToken.json $SOLIDITY_JSON_PATH
...
echo; echo == lock eth
echo yarn -s --cwd $BASEDIR/smart-contracts integrationtest:sendLockTx --gridchain_address $ROWAN_SOURCE \
  --symbol eth \
  --ethereum_private_key_env_var "ETHEREUM_PRIVATE_KEY" \
  --json_path $BASEDIR/smart-contracts/deployments/$DEPLOYMENT_NAME \
  --gas estimate \
...
echo yarn -s --cwd $BASEDIR/smart-contracts integrationtest:sendBurnTx \
  ...
  --bridgebank_address $BRIDGE_BANK_ADDRESS \
  --ethereum_address $ETHEREUM_ADDRESS \
  --gridchain_address $ROWAN_SOURCE \
  --amount 17
...
echo $gridnodecmd q auth account --node $GRIDNODE $ROWAN_SOURCE
...
echo; echo == send ceth
echo $gridnodecmd tx send $ROWAN_SOURCE gridsomedestination 100rowan \
  --node $GRIDNODE \
  --keyring-backend test \
  --fees 100000rowan \
  --chain-id=$DEPLOYMENT_NAME  \
...
echo TOKENS=ceth,rowan NTRANSFERS=2 python3 -m pytest -olog_level=DEBUG -olog_file=vagrant/data/pytest.log -v src/py/test_bulk_transfers_to_ethereum.py::test_bulk_transfers_from_gridchain
