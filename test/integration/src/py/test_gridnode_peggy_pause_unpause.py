...
def test_pause_lock_valid(ctx: EnvCtx):
    ...
    test_grid_account = ctx.create_gridchain_addr(fund_amounts=[[fund_amount_grid, "rowan"]])
    ...
    assert balance_diff.get(gridchain.ROWAN, 0) == (-1 * (send_amount + gridchain.grid_tx_fee_in_rowan )), "Gas fee and sent amount should be deducted from sender grid acct"
    ...
    balance_diff, erc_diff = send_test_account(ctx, test_grid_account, test_eth_destination_account, send_amount, erc20_token_addr=ctx.get_bridge_token_sc().address)
    assert balance_diff.get(gridchain.ROWAN, 0) == (-1 * gridchain.grid_tx_fee_in_rowan), "Only gas fee should be deducted for attempted tx"
...
def test_pause_burn_valid(ctx: EnvCtx):
    ...
    assert balance_diff.get(gridchain.ROWAN, 0) == (-1 * gridchain.grid_tx_fee_in_rowan), "Gas fee should be deducted from sender grid acct"
...
def send_test_account(ctx: EnvCtx, test_grid_account, test_eth_destination_account, send_amount, denom=gridchain.ROWAN, erc20_token_addr: str=None) -> Tuple[cosmos.Balance, int]:
