...
from test_utilities import get_required_env_var, get_shell_output, GridchaincliCredentials
...
def build_request(
    ...
    request = EthereumToGridchainTransferRequest(
...
def test_transfer_eth_to_ceth_using_replay_blocks(
    ...
    rowan_transfer_request.gridchain_symbol = "rowan"
    small_amount = 9
    rowan_transfer_request.amount = small_amount
    test_utilities.send_from_gridchain_to_ethereum(rowan_transfer_request, credentials)
    ...
    logging.info(f"send {small_amount} rowan to {request.gridchain_address}")
