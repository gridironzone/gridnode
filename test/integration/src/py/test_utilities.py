...
@dataclass
class EthereumToGridchainTransferRequest:
    ...
    amount: int = 0
    ceth_amount: int = 0
    gridchain_fees: str = ""  # Deprecated, see https://github.com/Gridchain/gridnode/pull/1802#discussion_r697403408
    ...
    @staticmethod
    def from_args(args):
        return EthereumToGridchainTransferRequest(
...
class RequestAndCredentials:
    transfer_request: EthereumToGridchainTransferRequest
    credentials: GridchaincliCredentials
    args: object
...
def get_eth_balance(transfer_request: EthereumToGridchainTransferRequest):
...
def get_whitelisted_tokens(transfer_request: EthereumToGridchainTransferRequest):
    network_element = f"--ethereum_network {transfer_request.ethereum_network} " if transfer_request.ethereum_network else ""
    symbol_element = f"--symbol {transfer_request.ethereum_symbol} " if transfer_request.ethereum_symbol else ""
    private_element = f"--ethereum_private_key_env_var \"{transfer_request.ethereum_private_key_env_var}\"" if transfer_request.ethereum_private_key_env_var else ""
    command_line = " ".join(
...
def mint_tokens(transfer_request: EthereumToGridchainTransferRequest, operator_address):
    network_element = f"--ethereum_network {transfer_request.ethereum_network} " if transfer_request.ethereum_network else ""
    symbol_element = f"--symbol {transfer_request.ethereum_symbol} " if transfer_request.ethereum_symbol else ""
    private_element = f"--ethereum_private_key_env_var \"{transfer_request.ethereum_private_key_env_var}\"" if transfer_request.ethereum_private_key_env_var else ""
    command_line = " ".join(
...
def wait_for_eth_balance(transfer_request: EthereumToGridchainTransferRequest, target_balance, max_seconds=80):
    wait_for_balance(
...
def send_from_gridchain_to_gridchain_cmd(
    ...
    gridchain_fees_entry = f"--fees {transfer_request.gridchain_fees}" if transfer_request.gridchain_fees else ""  # Deprecated, see https://github.com/Gridchain/gridnode/pull/1802#discussion_r697403408
...
def send_from_gridchain_to_ethereum_cmd(
    ...
    """
    Sends from Gridchain to Ethereum.
    ...
    node = f"--node {transfer_request.gridnoded_node}" if transfer_request.gridnoded_node else ""
    gridchain_fees_entry = f"--fees {transfer_request.gridchain_fees}" if transfer_request.gridchain_fees else ""  # Deprecated, see https://github.com/Gridchain/gridnode/pull/1802#discussion_r697403408
    direction = "lock" if transfer_request.gridchain_symbol == "rowan" else "burn"
    home_entry = f"--home {credentials.gridnoded_homedir}" if credentials.gridnoded_homedir else ""
...
def send_from_gridchain_to_ethereum(transfer_request: EthereumToGridchainTransferRequest,
                                   credentials: GridchaincliCredentials):
...
def send_from_ethereum_to_gridchain(transfer_request: EthereumToGridchainTransferRequest) -> int:
    direction = "sendBurnTx" if transfer_request.gridchain_symbol == "rowan" else "sendLockTx"
...
def wait_for_ethereum_block_number(block_number: int, transfer_request: EthereumToGridchainTransferRequest):
    network_element = f"--ethereum_network {transfer_request.ethereum_network} " if transfer_request.ethereum_network else ""
...
def approve_token_amount(token_request: EthereumToGridchainTransferRequest):
