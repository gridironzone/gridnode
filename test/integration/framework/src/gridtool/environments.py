...
from gridtool.gridchain import ROWAN, ROWAN_DECIMALS, STAKE
...
class GridnodedEnvironment:
    ...
    def _update_configuration_files(self, node_info, peers_node_info):
        ...
        app_toml["minimum-gas-prices"] = grid_format_amount(0.5, ROWAN)
    ...
    def _broadcast_create_validator_msg(self, node_info: JsonDict):
        ...
        assert cosmos.balance_exceeds(admin_balance, {ROWAN: gridchain.grid_tx_fee_in_rowan}), \
            "Validator admin {} needs at least one grid_tx_fee_in_rowan to fund the transaction".format(admin_addr)
    ...
    def setup_liquidity_pools_simple(self, pool_definitions: Mapping[str, Tuple[int, int, int]]):
        ...
        if self.faucet != self.clp_admin:
            ...
            gridnoded.send_batch(self.faucet, self.clp_admin, cosmos.balance_add(
                ...
                {gridchain.ROWAN: sum(native_amount for _, (_, native_amount, _) in pool_definitions.items())}))
        ...
        #     "permissions": [1]
        # }
        gridnoded.token_registry_register(gridnoded.create_tokenregistry_entry(ROWAN, ROWAN, ROWAN_DECIMALS, rowan_permissions),
