...
from gridtool.gridchain import Gridgen, Gridnoded, Ebrelayer, gridchain_denom_hash, ROWAN
...
class Integrator(Ganache, Command):
    ...
    def gridchain_init_integration(self, gridnode, validator_moniker, validator_mnemonic, denom_whitelist_file):
        ...
        tokens = {ROWAN: 10 ** 28}
        # Original from peggy:
        # self.cmd.execst(["gridnoded", "add-genesis-account", gridnoded_admin_address, "100000000000000000000rowan", "--home", gridnoded_home])
...
class UIStackEnvironment:
    ...
    def stack_save_snapshot(self):
        ...
        gridnoded_proc = self.cmd.gridnoded_start(minimum_gas_prices=(0.5, ROWAN))  # TODO gridnoded_home=???
        ...
        gridnode.tx_clp_create_pool(self.chain_id, self.keyring_backend, "akasha", "catk", [10**5, ROWAN], 10**25, 10**25)
        time.sleep(5)
        log.info("Creating liquidity pool from cbtk:rowan...")
        gridnode.tx_clp_create_pool(self.chain_id, self.keyring_backend, "akasha", "cbtk", [10**5, ROWAN], 10**25, 10**25)
        # should now be able to swap from catk:cbtk
        time.sleep(5)
        log.info("Creating liquidity pool from ceth:rowan...")
        gridnode.tx_clp_create_pool(self.chain_id, self.keyring_backend, "akasha", "ceth", [10**5, ROWAN], 10**25, 83*10**20)
        # should now be able to swap from x:ceth
        time.sleep(5)
        log.info("Creating liquidity pool from cusdc:rowan...")
        gridnode.tx_clp_create_pool(self.chain_id, self.keyring_backend, "akasha", "cusdc", [10**5, ROWAN], 10**25, 10**25)
        time.sleep(5)
        log.info("Creating liquidity pool from clink:rowan...")
        gridnode.tx_clp_create_pool(self.chain_id, self.keyring_backend, "akasha", "clink", [10**5, ROWAN], 10**25, 588235*10**18)
        time.sleep(5)
        log.info("Creating liquidity pool from ctest:rowan...")
        gridnode.tx_clp_create_pool(self.chain_id, self.keyring_backend, "akasha", "ctest", [10**5, ROWAN], 10**25, 10**13)
...
class IntegrationTestsEnvironment:
    ...
    def run(self):
        ...
        # Start gridnoded
        gridnoded_proc = gridnode.gridnoded_start(tcp_url=self.tcp_url, minimum_gas_prices=(0.5, ROWAN),
    ...
    def restart_processes(self):
        ...
        gridnoded_home = os.path.join(chaindir, ".gridnoded")
        gridnoded_proc = self.cmd.gridnoded_start(tcp_url=self.tcp_url, minimum_gas_prices=(0.5, ROWAN),
        ...
        for config in launch_json["configurations"]:
            if config["name"].startswith("Debug Relayer"):
                intellij_ebrelayer_configs.append(render_intellij_run_xml(
                    "ebrelayer devenv",
                    " ".join(config["args"]),
                    "github.com/Gridchain/gridnode/cmd/ebrelayer",
                ...
                intellij_gridnoded_configs.append(render_intellij_run_xml(
                    "gridnoded devenv",
                    " ".join(config["args"]),
                    "github.com/Gridchain/gridnode/cmd/gridnoded",
