...
from gridtool.gridchain import ROWAN, CETH
...
gridnode_funds_for_transfer_peggy1 = 10**17  # rowan
...
def get_env_ctx(cmd=None, env_file=None, env_vars=None):
    ...
    # Add any Gridchain private keys to test keystore
...
def get_env_ctx_peggy2():
    ...
    if "GRIDTOOL_ENV_FILE" in os.environ:
        ...
        rowan_source = gridchain_config.get("rowan_source")
        if not rowan_source:
            if "rowan_source_mnemonic" in gridchain_config:
                rowan_source_mnemonic = gridchain_config["rowan_source_mnemonic"]
            ...
            if rowan_source_mnemonic:
                ...
                rowan_source = gridchain.mnemonic_to_address(cmd, rowan_source_mnemonic)
                gridnoded = gridchain.Gridnoded(cmd, home=gridnoded_home)
                if not [x for x in gridnoded.keys_list() if x["address"] == rowan_source]:
                    gridnoded.keys_add(None, rowan_source_mnemonic)
...
def get_env_ctx_peggy1(cmd=None, env_file=None, env_vars=None):
    ...
    generic_erc20_contract_name = "GridchainTestToken"
    if "SMART_CONTRACT_ARTIFACT_DIR" in env_vars:
        ...
        if deployment_name == "gridchain-1":
            # Special case for Betanet because GridchainTestToken is not deployed there.
...
class EnvCtx:
    def __init__(self, cmd: command.Command, w3_conn: web3.Web3, ctx_eth: eth.EthereumTxWrapper, abi_provider,
        ...
        self.gridchain_ethbridge_admin_account = self.rowan_source
    ...
    def tx_deploy_new_generic_erc20_token(self, deployer_addr: str, name: str, symbol: str, decimals: int, cosmosDenom: str = None) -> Contract:
        # return self.tx_deploy("GridchainTestToken", self.operator, [name, symbol, decimals])
        if on_peggy2_branch:
            # Use BridgeToken
            assert self.generic_erc20_contract == "BridgeToken"
            if cosmosDenom is None:
                cosmosDenom = "erc20denom"  # TODO Dummy variable since we're using BridgeToken instead of GridchainTestToken

            constructor_args = [name, symbol, decimals, cosmosDenom]
        else:
            # Use GridchainTestToken for TestNet and Devnet, and BridgeToken for Betanet
    ...
    def create_new_currency(self, symbol, name, decimals, amount, minted_tokens_recipient):
        ...
        1. Deploys a new instance of GridchainTestToken
        ...
        """
        assert self.generic_erc20_contract == "GridchainTestToken"  # Preserve compatibiliy with integration tests and inflate_tokens.sh
    ...
    def create_gridchain_addr(self, moniker: str = None,
        ...
        if fund_amounts:
            ...
            rowan_source_balances = self.get_gridchain_balance(self.rowan_source)
            for denom, required_amount in fund_amounts.items():
                ...
                assert available_amount >= required_amount, "Rowan source {} would need {}, but only has {}".format(
                    self.rowan_source, grid_format_amount(required_amount, denom), grid_format_amount(available_amount, denom))
            old_balances = self.get_gridchain_balance(grid_address)
            self.send_from_gridchain_to_gridchain(self.rowan_source, grid_address, fund_amounts)
    ...
    def bridge_bank_lock_eth(self, from_eth_acct, to_grid_acct, amount):
        """ Sends ETH from Ethereum to Gridchain (lock) """
    ...
    # For Gridchain-native assets (rowan) we need to use "burn".
    # Compare: smart-contracts/scripts/test/{sendLockTx.js OR sendBurnTx.js}
    # sendBurnTx is called when gridchain_symbol == "rowan", sendLockTx otherwise
    ...
    def sanity_check(self):
        ...
        if len(rowan_source_account) != 1:
            raise Exception
        rowan_source_balance = self.get_gridchain_balance(self.rowan_source).get(ROWAN, 0)
