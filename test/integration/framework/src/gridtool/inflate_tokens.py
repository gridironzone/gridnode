...
class InflateTokens:
    ...
    def build_list_of_tokens_to_create(self, existing_tokens: Iterable[TokenDict], requested_tokens: Iterable[TokenDict]
        ...
        """
        This part deploys GridchainTestoken for every requested token that has not yet been deployed.
    ...
    def transfer(self, requested_tokens: Sequence[TokenDict], token_amount: int,
        ...
        The grid_broker_account and OPERATOR can be any Gridchain and Ethereum accounts, we might want to use something
        ...
        fund_rowan = [5 * test_utils.gridnode_funds_for_transfer_peggy1 * n_accounts * number_of_batches, "rowan"]
        ...
        grid_broker_account = self.ctx.create_gridchain_addr(fund_amounts=[fund_rowan])
        ...
        rowan_source_key = zero_or_one([k for k in keys if k["address"] == grid_broker_account])
        assert rowan_source_key is not None, "Need private key of broker account {} in gridnoded test keyring".format(grid_broker_account)
