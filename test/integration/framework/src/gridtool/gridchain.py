...
# Gridchain public network endpoints
...
grid_tx_fee_in_rowan = 1 * 10**17

# Fees for "ethbridge burn" transactions. Determined experimentally
# TODO This should be dynamic (per-Gridnoded)
grid_tx_burn_fee_in_rowan = 100000
grid_tx_burn_fee_in_ceth = 1

# TODO This should be dynamic (per-Gridnoded)
# There seems to be a minimum amount of rowan that a grid account needs to own in order for the bridge to do an
# "ethbridge burn". This amount does not seem to be actually used. For example, if you fund the account just with
# grid_tx_burn_fee_in_rowan, We observed that if you try to fund grid accounts with just the exact amount of rowan
# needed to pay fees (grid_tx_burn_fee_in_rowan * number_of_transactions), the bridge would stop forwarding after
...
# TODO This should be dynamic (per-Gridnoded)
grid_tx_burn_fee_buffer_in_rowan = 5 * grid_tx_fee_in_rowan
...
class Gridnoded:
    def __init__(self, cmd, /, home: Optional[str] = None, node: Optional[str] = None, chain_id: Optional[str] = None,
        ...
        self.fees = grid_tx_fee_in_rowan
        self.gas = None
    ...
    def send_and_check(self, from_addr: cosmos.Address, to_addr: cosmos.Address, amounts: cosmos.Balance
        ...
        expected_tx_fee = {ROWAN: grid_tx_fee_in_rowan}
    ...
    # See https://github.com/Gridchain/gridnode/blob/master/docs/tutorials/clp%20tutorial.md
    ...
    def _fees_args(self) -> List[str]:
        return ["--fees", grid_format_amount(self.fees, ROWAN)]
...
class GridnodeClient:
    ...
    def send_from_gridchain_to_ethereum(self, from_grid_addr: cosmos.Address, to_eth_addr: str, amount: int, denom: str,
        ...
        """ Sends ETH from Gridchain to Ethereum (burn) """
        assert self.ctx.eth
        eth = self.ctx.eth
    ...
    def open_grpc_channel(self) -> grpc.Channel:
        ...
        # See https://raw.githubusercontent.com/Gridchain/gridchain-ui/develop/ui/core/swagger.yaml
