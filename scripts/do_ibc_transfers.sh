...
gridnoded tx ibc-transfer transfer transfer channel-1 $(gridnoded keys show grid -a --keyring-backend=test --home ~/.gridnode-1) 50000000000000000000rowan --node tcp://127.0.0.1:27666 --chain-id=localnet-2 --from=akasha --log_level=debug  --keyring-backend test --gas-prices 10000000000000000rowan  --home ~/.gridnode-2 --yes --broadcast-mode block
...
gridnoded tx ibc-transfer transfer transfer channel-0 $(gridnoded keys show grid -a --keyring-backend=test --home ~/.gridnode-1) 50000000000000000000rowan --node tcp://127.0.0.1:27665 --chain-id=localnet-1 --from=akasha --log_level=debug  --keyring-backend test --gas-prices 10000000000000000rowan  --home ~/.gridnode-1 --yes --broadcast-mode block
echo "Tried localnet-1 -> localnet-2"
echo ""

sleep 5

gridnoded tx ibc-transfer transfer transfer channel-1 $(gridnoded keys show grid -a --keyring-backend=test --home ~/.gridnode-1) 50000000000000000000rowan --node tcp://127.0.0.1:27667 --chain-id=localnet-3 --from=akasha --log_level=debug  --keyring-backend test --gas-prices 10000000000000000rowan  --home ~/.gridnode-3 --yes --broadcast-mode block
...
echo "Current Grid balances (should go up for rowan)"
echo "localnet-1"
